...
void CopyFileContent(char Source[],char Dest[],char Buff[],long n,long offset)
{
    ...
    while((n = pread(fd1,Buff,size,offset)) > 0)
    {
        if(pwrite(fd2,Buff,n,offset) != n)      // write exactly n bytes to destination file
        {
            perror("write");
            close(fd1);
            close(fd2);
...
int main(int argc , char *argv[])
{
    ...
    CopyFileContent(argv[1],argv[2],Buff,n,0);
